...
def test_dethrone_no_counter_if_not_highest():
    ...
    attacker = CombatCreature("Challenger", 2, 2, "A", dethrone=True)
    defender = CombatCreature("Dummy", 0, 1, "B")
    state = GameState(players={"A": PlayerState(life=20, creatures=[attacker]), "B": PlayerState(life=15, creatures=[defender])})
    sim = CombatSimulator([attacker], [defender], game_state=state)
    sim.simulate()
    assert attacker.plus1_counters == 0
...
def test_dethrone_triggers_when_tied_for_highest():
    ...
    attacker = CombatCreature("Challenger", 2, 2, "A", dethrone=True)
    defender = CombatCreature("Dummy", 0, 1, "B")
    state = GameState(players={"A": PlayerState(life=20, creatures=[attacker]), "B": PlayerState(life=20, creatures=[defender])})
    sim = CombatSimulator([attacker], [defender], game_state=state)
    sim.simulate()
...
def test_dethrone_counter_even_if_blocked():
    ...
    link_block(atk, blk)
    state = GameState(players={"A": PlayerState(life=20, creatures=[atk]), "B": PlayerState(life=25, creatures=[blk])})
    sim = CombatSimulator([atk], [blk], game_state=state)
    sim.simulate()
    assert atk.plus1_counters == 1
    assert blk in sim.finalize().creatures_destroyed if hasattr(sim, 'finalize') else True
...
def test_two_dethrone_creatures_each_get_counter():
    ...
    defender = CombatCreature("Dummy", 0, 1, "B")
    state = GameState(players={"A": PlayerState(life=20, creatures=[atk1, atk2]), "B": PlayerState(life=25, creatures=[defender])})
